...
2. **The `procurement.*` clause enum primarily _structures the reason
   axis_ that OCDS leaves unstructured.** Most clauses align to
...
| UBL `cac:ProcessJustification/cbc:PreviousCancellationReasonCode` and `cbc:ProcessReasonCode` | These justify the _choice of procurement procedure_ (e.g. why direct award vs open competition), not bid-level rejection.                                                                                              | Out of scope for this artifact; would belong on the canonical opportunity fragment if we ever needed it.                                    |
...
4. **Single-string `clauseDescription` text alongside the enum?** OCDS
   ...
   alias to that on emit, we should also accept _receiving_ prose from
...
- **`bid_not_most_economically_advantageous` (#8)** is the one
  ...
  proxies are inverse (the _winner's_ award status is `active`; the
  _losers'_ state is mute) or indirect (evaluation reports as a
...
**not bid-level**. It captures _why the whole procurement did not
result in an award_: `all-rej` (all tenders inadmissible),
